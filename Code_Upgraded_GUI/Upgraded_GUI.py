...
from ultrasonic_data_extractor_upgraded import RedPitayaSensor
import time
...
class EnhancedRedPitayaSensor(RedPitayaSensor):
    ...
    def start_sensor(self):
        ...
        self.give_ssh_command(full_command)

    def stop_sensor(self):
        command = "pidof dma_with_udp"
        pid = self.give_ssh_command(command)
        command1 = f"kill {pid}"
        self.give_ssh_command(command1)
...
@profile
def process_and_plot_signal(temperature_var, sampling_frequency_var, peak_detection_method, yanowitz_frame, niblack_frame, master, output_text, selected_signal, sensor):
    clear_plot(master)
    ...
    # Slice the data from index 5000 to 75000 and reset the index
    raw_adc = raw_adc[5000:75000].reset_index(drop=True)
...
def main():
    ...
    start_sensor_button = Button(root, text="Start Sensor", command=lambda: sensor.start_sensor())
    start_sensor_button.pack(side=TOP)

    stop_sensor_button = Button(root, text="Stop Sensor", command=lambda: sensor.stop_sensor())
    stop_sensor_button.pack(side=TOP)
    ...
    start_data_button = Button(root, text="Start Data Acquisition", command=lambda: extract_data(sensor, output_text))
    start_data_button.pack(side=TOP)
